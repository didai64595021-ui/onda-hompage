...
2. **관리자가 없는 경우** → 모든 메시지에 "⛔ 관리자 승인이 필요합니다. 관리자(@gumibear012)를 그룹에 초대해 주세요." 응답 후 무시
3. **관리자가 있는 경우** → 관리자에게 승인 요청: "🔐 새 그룹 감지: {그룹명}. 봇을 활성화할까요? (Y/N)"
4. **관리자가 승인("Y", "ㅇ", "확인", "ㄱ")** → 봇 활성화, 정상 작동 시작
5. **관리자가 거부("N", "ㄴ", "취소")** → 봇 비활성 유지
...
#### 미승인 그룹에서의 동작
- 모든 메시지 무시 (HEARTBEAT_OK 포함)
- 단, 관리자 메시지는 항상 수신하여 승인 여부 확인
