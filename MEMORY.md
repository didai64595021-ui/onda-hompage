...
- 고객이 물어본 내용에 대한 답변을 먼저 해주고 질문으로 이어감 (맥락 유지)
- **상품 선정 질문** (자영업자가 이해 가능한 수준):
  - "한 페이지에 다 담을까요, 메뉴별로 나눌까요?"
  - "납품 후 직접 수정하실 일이 있으실까요?"
  - "추가 기능 필요한 게 있으시면 알려주세요"
- **상품 선정 기준** (내부용):
  - 1P + 직접수정X → STANDARD (12만)
  - 1P + 직접수정O 또는 2~3P → DELUXE (20만, CMS포함)
  - 4P+ 또는 유지보수 필요 → PREMIUM (35만, CMS+유지보수1개월)
- 가격 확정 항목은 "포함입니다" 단언
- 호스팅/기술 용어 묻지 말고 알아서 처리
- 이모지 금지 → :) 텍스트 이모티콘만
- **3개 패키지**: STANDARD 12만(1P/3일), DELUXE 20만(3P/CMS/5일), PREMIUM 35만(5P/CMS/유지보수1개월/7일)
- **추가 옵션**: 서브페이지+2만, 카카오채널+3만, 네이버예약+3만, 배너+3만, QR+3만, 인스타+5만, SEO심화+5만
