...
- **스크립트 파일**: `docs/KMONG_SCRIPT.md` — 문의 올 때마다 참조
- **구조**: 인사 + 고객 문의사항 답변 + 기본 질문 4개 (한 메시지로)
- 고객이 물어본 내용에 대한 답변을 먼저 해주고, 기본 질문으로 이어감 (맥락 유지)
- 질문 4개: 업체명/대표자명/전화번호, 서비스 지역, 주요 서비스 항목, 로고/현장사진
- 가격 확정 항목은 "포함입니다" 단언 (견적 언급 X)
- 호스팅/기술 용어 묻지 말고 알아서 처리
- 행동 유도: "편하게~" → "주시면 바로 착수합니다"
- 이모지 금지 → :) 텍스트 이모티콘만
- 10줄 이내 (합친 메시지는 약간 초과 허용)
