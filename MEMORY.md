...
## AI 이미지 생성 필수 규칙 (2026-04-06, 영구, 최우선)
- **모든 AI 생성 이미지 사용 전 반드시 텍스트 깨짐 검증 필수**
- 방법: image 분석 tool로 "모든 텍스트 읽어줘, 깨진 글자 있는지 확인" 요청
- 깨짐 발견 시 → 재생성 (최대 3회) → 통과된 것만 업로드
- 특히 한글+영문 혼용 이미지에서 AI가 한글 중간에 영문 조합 넣는 경우 있음 (예: "매킷경링" = "마케팅" 깨짐)
- 크몽 업로드 전 100% 필수 통과 조건

## 크몽 자동 최적화 보고 형식 (2026-04-06, 영구)
- 교체 발생 시 항상 아래 형식으로 텔레그램 보고:
```
📋 [상품명] 자동 교체 보고
- 제목: 이전 → 새것
- 설명: 주요 변경 포인트
...
- 반영 상태: 라이브 확인 완료
```
- 자동/수동 교체 모두 적용, 생략 금지

## 크몽 로그인 방식 — 절대 수정 금지 (2026-04-04, 영구, 최최우선)
- **유일한 로그인 방식**: 메인(kmong.com) → 헤더 "로그인" 버튼 클릭 → 모달 로그인
- **/login URL = 404** (존재하지 않음)
- **/?open=login_modal = 404** (작동 안 함)
- **kmong-crawler/lib/login.js 파일 변경 금지** — 다른 에이전트/서브에이전트가 이 파일을 변경하면 크롤러 6개 동시 장애
- 이미 4회 복원함 (2026-04-03~04) — 다른 프로세스가 반복적으로 /?open=login_modal 방식으로 되돌림
...
4. **코딩은 Claude Opus 4.7에 위임** — 직접 코딩 X, coding-agent 스킬로 위임.
