...
- **3단계 플로우 (영구)**:
  1. **질문** — 고객 문의사항 답변 + 기본정보 질문 + 견적 관련 옵션 질문
  2. **견적** — 답변 기반 견적 산출 안내 (기본+옵션 합계)
  3. **착수** — 결제 확인 후 착수 안내
- **절대 1단계에서 "바로 착수" 하지 마라** — 질문→견적→착수 순서 필수
- 고객이 물어본 내용에 대한 답변을 먼저 해주고 질문으로 이어감 (맥락 유지)
- 질문: 업체명/대표자명/전화번호, 서비스 지역, 주요 서비스 항목, 로고/현장사진, 추가 옵션 선택
- 가격 확정 항목은 "포함입니다" 단언
- 호스팅/기술 용어 묻지 말고 알아서 처리
- 이모지 금지 → :) 텍스트 이모티콘만
- **기본 포함(12만)**: 반응형 1페이지, 전화+카카오, 견적문의 버튼/팝업, 지도, SSL, 검색등록, 수정무제한, 7일AS, 도메인/호스팅
- **추가 옵션**: 서브페이지+2만, 카카오채널+3만, 네이버예약+3만, 배너+3만, QR+3만, CMS+5만, 인스타+5만, SEO심화+5만
