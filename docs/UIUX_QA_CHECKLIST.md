...
## 11. 실제 모바일 렌더링 필수 체크 (2026-03-12 추가, 관리자 피드백 기반)

### 히어로 영역
- [ ] background-clip:text 사용 금지 (모바일 깨짐) → 대신 배경이미지+오버레이+일반텍스트
- [ ] 히어로 상단 빈 공간 과다 금지 (nav 높이 + 적절한 여백만)
- [ ] 마우스 스크롤 아이콘 → 모바일에서는 숨기거나 스와이프 힌트로 대체
- [ ] 히어로 텍스트 최소 크기 모바일 clamp(2rem, 10vw, 4rem)

### 햄버거 메뉴
- [ ] X(닫기) 버튼 반드시 있어야 함 — 햄버거→X 토글 변환 필수
- [ ] 메뉴 항목 좌측 쏠림 금지 → text-align:center 또는 적절한 padding
- [ ] 현재 페이지 active 표시 (색상/밑줄)
- [ ] 메뉴 열린 상태에서 body overflow:hidden
- [ ] 하단에 연락처/SNS 링크 배치 (빈 공간 활용)

### 카드/그래프
- [ ] 원형 그래프: 컨테이너 overflow 확인 → 잘림 방지
- [ ] 그래프 바: 실제 % 데이터 반영 (0%/100%만 아닌 실제 수치)
- [ ] 카드 텍스트: overflow:hidden + text-overflow:ellipsis 또는 충분한 높이
- [ ] Before/After 비교 카드: 모바일에서 가로 배치 → 세로 배치 전환
- [ ] 카드 내 금액/숫자 절대 잘림 금지 (min-width 또는 줄바꿈)

### 이미지+텍스트 겹침
- [ ] 이미지 위 텍스트: 반드시 오버레이(gradient/solid) 배경 추가
- [ ] 최소 대비 4.5:1 확보
- [ ] 배경 이미지 밝은 부분 대비: text-shadow 또는 반투명 배경 필수

### 푸터
- [ ] 모바일에서 메뉴 한 줄 넘어가면 줄바꿈 처리 (flex-wrap:wrap)
- [ ] 우측 잘림 금지 (padding-right 충분히)
- [ ] 메뉴 항목 간격 균등 (gap 사용)

### 폼 (Contact)
- [ ] 제출 버튼: width:100% 또는 text-align:center (좌측 치우침 금지)
- [ ] 선택형 버튼: 선택 상태 시각적 구분 (배경색 변경)
- [ ] textarea: min-height:120px (높이 충분)
- [ ] 폼 필드 간 여백 통일 (모두 동일한 gap)
- [ ] 폼과 푸터 사이 여백 충분 (min 48px)

### 페이지 타이틀
- [ ] 웹폰트 로딩 실패 시 fallback 정상 표시
- [ ] 타이틀이 네비게이션과 겹치지 않도록 padding-top 충분
