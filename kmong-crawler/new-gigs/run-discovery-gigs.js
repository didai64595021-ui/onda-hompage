...
const IN_PATH = path.join(__dirname, 'discovery-10-full.json');
const OUT_PATH = path.join(__dirname, 'run-discovery-log.json');
const IMAGES_DIR = path.join(__dirname, '03-images-discovery');

function toProduct(gig, idx) {
  // 이미지 파일명만 지정 — create-gig.js 의 IMAGE_DIR(03-images) 과 조합되어 사용
  const image = `${gig.slug}.png`;
  return {
