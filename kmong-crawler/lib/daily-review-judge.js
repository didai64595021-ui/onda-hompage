...
## 출력 JSON (텍스트 금지)
{
  ...
  "candidate_actions": [
    {"action":"hold|re_run_ad_bot|raise_budget|lower_budget|pause_product|alert_only", "target":"product_id or ALL", "value": number or null, "reason":"..."}
  ],
  "verdict_summary": "전체 한줄 평"
}

## ★ action·target·value 의미 (혼동 금지)
- **raise_budget / lower_budget**: \`value\` = **주간 총 예산 금액(원)**. 일예산·CPC 아님
  - target="ALL" → 전체(product_id IS NULL) 예산 행 조정
  - target=product_id → 해당 상품 전용 예산 행 조정 (없으면 생성)
  - 현재 전체 주예산은 입력 context.budget 참고. 전체를 건드리지 말라고 하려면 반드시 target=product_id 명시
  - CPC/키워드 조정이 목적이면 **lower_budget 쓰지 말고 re_run_ad_bot** 선택
- **re_run_ad_bot**: ad-bot-run.js --apply 재실행 → CPC/키워드 Opus 재판단·적용
- 안전장치: 전체 예산 ±50% 초과 변경은 실행부에서 자동 거부됨`;
...
## ★ pause_product 절대 금지 조건
- orders_30d ≥ 1 서비스 → pause_product 제안 즉시 **re_run_ad_bot(세분화 재조정)** 또는 **hold**로 downgrade
- 완전 OFF는 orders_30d = 0 AND cost_30d ≥ 50000 AND 2주 연속 음수 ROI 에서만 고려
- 근거 불확실하면 alert_only (사람 판단)로 전환

## ★ raise_budget / lower_budget 의미 명시 (혼동 금지)
- \`value\` = **주간 총 예산 금액(원)**. 일예산·CPC·특정 상품 일한도 아님
- target="ALL" → 전체(product_id IS NULL) 예산 행
- target=product_id → 해당 상품 전용 행 (없으면 신규 생성)
- **상품의 CPC/키워드를 조정하려는 의도면 re_run_ad_bot으로 전환**. lower_budget은 총 예산 자체를 줄이려 할 때만
- 전체 예산 ±50% 초과 변경은 실행부가 자동 거부 → 필요 시 사람 확인 요청(alert_only) 권고
