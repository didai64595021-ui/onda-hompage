...
## 단가 기반 손익 가드 (★ 모든 Phase에서 최우선 적용)
서비스별 메트릭에 다음 필드가 들어옵니다:
- price_min / price_max: 패키지 가격 범위 (원)
- avg_order_value: 1주문 평균 매출 (실측 revenue_30d/orders_30d, 없으면 price_min fallback)
- expected_margin_per_click: 1클릭당 기대 마진 = AOV × 0.4(마진율) × cvr_inquiry × 0.3(inq→order)
- deficit_risk: desired_cpc / expected_margin_per_click — **1.0 이상이면 클릭 1번당 적자**

규칙:
- **deficit_risk ≥ 1.0** → suggested_cpc 무조건 ≤ expected_margin_per_click × 0.8. 상향 금지. Phase가 volume이라도.
- **deficit_risk 0.7~1.0** → 동결 또는 -5% 하향. 키워드 정리 위주(disable 저성과).
- **deficit_risk < 0.5** → 정상. Phase 가드대로 운용.
- **avg_order_value < 100,000원**(저단가 상품) → max suggested_cpc = AOV × 0.05 (5%) 자체 캡 적용.
- **avg_order_value ≥ 500,000원**(고단가) → Phase 가드 안에서 더 공격적 상향 OK.
- expected_margin_per_click이 null(데이터 부족)이면 보수적: ±10% 이내만.
reasoning에 "AOV/마진/적자위험" 한 줄 명시 권장.
...
async function judgeAdjustments(metrics, budget) {
  ...
  for (const a of parsed.actions) {
    ...
    const m = metricsByPid[a.product_id];
    if (m) {
      const emc = m.expected_margin_per_click;
      const aov = m.avg_order_value;
      // 1) deficit_risk ≥ 1.0: 절대 상향 금지, 마진의 80% 이하로 캡
      if (emc && emc > 0 && cur > 0) {
        const deficitCap = Math.floor(emc * 0.8 / 10) * 10;
        if (clipped > cur && clipped > deficitCap) {
          clipped = Math.min(clipped, Math.max(deficitCap, cur));
          a.deficit_guard_applied = true;
        }
      }
      // 2) 저단가 상품(AOV<10만원): suggested ≤ AOV × 5%
      if (aov && aov < 100000) {
        const lowAovCap = Math.floor(aov * 0.05 / 10) * 10;
        if (clipped > lowAovCap && lowAovCap > 0) {
          clipped = lowAovCap;
          a.low_aov_guard_applied = true;
        }
      }
      a.deficit_risk_input = m.deficit_risk;
      a.expected_margin_per_click = emc;
      a.avg_order_value = aov;
    }
    // === /손익 하드 가드 ===
